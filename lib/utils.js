...
function signHostname(caKey, caCert, hostname){
    var key = pki.privateKeyFromPem(caKey);
    var cert = pki.certificateFromPem(caCert);
    var newCert = pki.createCertificate();

    newCert.publicKey = pki.setRsaPublicKey(key.n, key.e);
    newCert.serialNumber = '0x' + crypto.createHash('md5').update(hostname, 'utf8').digest('hex')

    newCert.validity.notBefore = new Date();
    newCert.validity.notAfter = new Date();
    newCert.validity.notAfter.setFullYear(newCert.validity.notBefore.getFullYear() + 20);
    ...
    newCert.setIssuer(cert.subject.attributes);

    newCert.sign(key, forge.md.sha256.create());

    return pki.certificateToPem(newCert);
}
