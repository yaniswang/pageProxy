    ...
    if(self._httpsServer === null){
        ...
        var httpsServer = https.createServer({
            ...
            SNICallback: function(hostname, callback){
                callback(null, mapCerts[hostname]);
            }
        },function(req, res){
...
PageProxyPrototype._getCert = function(hostname, callback){
    ...
    var cert;
    if(keyPath && fs.existsSync(certFile)){
        cert = fs.readFileSync(certFile).toString('utf-8');
    }
    else{
        cert = utils.signHostname(caKey, caCert, hostname);
        if(keyPath){
    ...
    mapCerts[hostname] = credentials.context;
}
...
// close proxy server
PageProxyPrototype.close = function(callback){
    var self = this;
    var httpServer = self._httpServer;
