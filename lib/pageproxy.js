...
PageProxyPrototype._doResponse = function(clientRequest, clientResponse){
    ...
    clientRequest.on('end', function () {
        ...
        new HttpFilter(arrFilters, httpData, function(httpData){
    ...
    remoteRequest.on('response', function (remoteResponse) {
        ...
        remoteResponse.on('end', function () {
            ...
            if(endCallback){
                ...
                if(self.bGUnzip === true && /^gzip$/i.test(remoteResponse.headers['content-encoding']) === true){
                    zlib.gunzip(responseBuffer, function(err, unzipBuffer){
                        delete remoteResponse.headers['content-encoding'];
                        remoteResponse.headers['content-length'] = unzipBuffer.length;
                        endCallback(remoteResponse.statusCode, remoteResponse.headers, unzipBuffer, {
